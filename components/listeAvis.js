...
export default function ListeAvisComponent() {
  ...
  const [guestName, setGuestName] = useState(""); // Pour les utilisateurs non connectés
  ...
  const handleSubmit = async (e) => {
    ...
    // Si ce champ caché est rempli, c'est un bot automatisé.
    if (honeypot.trim() !== "") {
      console.warn("Spam détecté via le honeypot !");
      // On réinitialise et on simule un succès pour tromper le bot sans enregistrer
      setShowForm(false);
      setDescription("");
      return;
    }
    ...
    // Nom affiché : soit le username de la session, soit la saisie de l'invité, soit "Visiteur" par défaut
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/add/avis`, {
        ...
        body: JSON.stringify({
          ...
          description,
          date: new Date(),
          website: honeypot, // Transmis au backend si nécessaire
        }),
      });
      ...
      if (response.ok) {
        ...
        setGuestName("");
        setShowForm(false);
        // On recharge la liste en refaisant un fetch
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <main className="max-w-6xl w-full mx-auto pt-28 px-4 pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 pb-8 mb-10">
          ...
          {/* Le bouton d'ouverture est désormais toujours accessible */}
        ...
        {showForm && (
          <div className="mb-12 bg-white border border-gray-200 p-6 sm:p-8 rounded-2xl shadow-xl shadow-gray-200/50 max-w-2xl mx-auto transition-all">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* 🛑 PIÈGE HONEYPOT ANTI-SPAM (invisible aux humains) */}
